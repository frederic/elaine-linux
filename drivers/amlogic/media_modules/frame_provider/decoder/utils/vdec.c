...
int vdec_set_starttime(struct vdec_s *vdec, unsigned int starttime)
{
	int r = -1;

	pr_info("vdec_set_starttime, starttime: %d\n", starttime);
	if (vdec->set_starttime) {
		pr_info("have set_starttime\n");
		r = vdec->set_starttime(vdec, starttime);
		if ((r == 0) && (vdec->slave) && (vdec->slave->set_starttime))
			r = vdec->slave->set_starttime(vdec->slave,
					starttime);
	} else {
		pr_info("have no set_starttime, use start_time\n");
		vdec->start_time = starttime;
		vdec->enable_start_time = true;
		if (vdec->slave) {
			vdec->slave->start_time = starttime;
			vdec->slave->enable_start_time = true;
		}
		r = 0;
	}
	return r;
}
EXPORT_SYMBOL(vdec_set_starttime);
...
void vdec_poweron(enum vdec_type_e core)
{
	...
	if (core == VDEC_1) {
		/* vdec1 power on */
		if (is_power_ctrl_ver2) {
			pwr_result = power_ctrl_sleep(1, 1);
			if (pwr_result)
				pr_err("vdec-1 power on ctrl bit2 sleep fail.\n");
		} else {
			pwr_result = power_ctrl_sleep(1, 2);
			if (pwr_result)
				pr_err("vdec-1 power on ctrl bit2 sleep fail.\n");
			pwr_result = power_ctrl_sleep(1, 3);
			if (pwr_result)
				pr_err("vdec-1 power on ctrl bit3 sleep fail.\n");
		}
		...
		if (is_power_ctrl_ver2) {
			pwr_result = power_ctrl_iso(1, 1);
			if (pwr_result)
				pr_err("vdec-1 power on ctrl bit2 iso fail.\n");
		} else {
			pwr_result = power_ctrl_iso(1, 6);
			if (pwr_result)
				pr_err("vdec-1 power on ctrl bit6 iso fail.\n");
			pwr_result = power_ctrl_iso(1, 7);
			if (pwr_result)
				pr_err("vdec-1 power on ctrl bit27 iso fail.\n");
		}
		...
		if (has_vdec2()) {
			/* vdec2 power on */
			pwr_result = power_ctrl_sleep(1, 4);
			if (pwr_result)
				pr_err("vdec-2 power on ctrl bit4 sleep fail.\n");
			pwr_result = power_ctrl_sleep(1, 5);
			if (pwr_result)
				pr_err("vdec-2 power on ctrl bit5 sleep fail.\n");
			...
			pwr_result = power_ctrl_iso(1, 8);
			if (pwr_result)
				pr_err("vdec-2 power on ctrl bit8 iso fail.\n");
			pwr_result = power_ctrl_iso(1, 9);
			if (pwr_result)
				pr_err("vdec-2 power on ctrl bit9 iso fail.\n");
		...
		if (has_hdec()) {
			/* hcodec power on */
			if (is_power_ctrl_ver2) {
				pwr_result = power_ctrl_sleep(1, 0);
				if (pwr_result)
					pr_err("vdec-1 power on ctrl bit1 sleep fail.\n");
			} else {
				pwr_result = power_ctrl_sleep(1, 0);
				if (pwr_result)
					pr_err("vdec-1 power on ctrl bit0 sleep fail.\n");
				pwr_result = power_ctrl_sleep(1, 1);
				if (pwr_result)
					pr_err("vdec-1 power on ctrl bit1 sleep fail.\n");
			}
			...
			if (is_power_ctrl_ver2) {
				pwr_result = power_ctrl_iso(1, 0);
				if (pwr_result)
					pr_err("hdec power on ctrl iso fail.\n");
			} else {
				pwr_result = power_ctrl_iso(1, 4);
				if (pwr_result)
					pr_err("hdec power on ctrl bit4 iso fail.\n");
				pwr_result = power_ctrl_iso(1, 5);
				if (pwr_result)
					pr_err("hdec power on ctrl bit5 iso fail.\n");
			}
		}
		...
		if (has_hevc_vdec()) {
			...
			while (!hevc_fixed) {
				...
				if (is_power_ctrl_ver2) {
					...
					pwr_result = power_ctrl_sleep(1, 6);
					if (pwr_result)
						pr_err("hevc power on ctrl bit6 sleep fail.\n");
					pwr_result = power_ctrl_sleep(1, 7);
					if (pwr_result)
						pr_err("hevc power on ctrl bit7 sleep fail.\n");
				}
				...
				if (is_power_ctrl_ver2) {
					pwr_result = power_ctrl_iso(1, 2);
					if (pwr_result)
						pr_err("hevc power on ctrl iso fail.\n");
				} else {
						pwr_result = power_ctrl_iso(1, 10);
						if (pwr_result)
							pr_err("hevc power on ctrl bit10 iso fail.\n");
						pwr_result = power_ctrl_iso(1, 11);
						if (pwr_result)
							pr_err("hevc power on ctrl bit11 iso fail.\n");
				}
...
void vdec_poweroff(enum vdec_type_e core)
{
	...
	if (core == VDEC_1) {
		...
		/* enable vdec1 isolation */
		if (is_power_ctrl_ver2) {
			pwr_result = power_ctrl_iso(0, 1);
			if (pwr_result)
				pr_err("vdec1 power off ctrl iso fail.\n");
		} else {
			pwr_result = power_ctrl_iso(0, 6);
			if (pwr_result)
				pr_err("vdec1 power off ctrl bit6 iso fail.\n");
			pwr_result = power_ctrl_iso(0, 7);
			if (pwr_result)
				pr_err("vdec1 power off ctrl bit7 iso fail.\n");
		}
		/* power off vdec1 memories */
		WRITE_VREG(DOS_MEM_PD_VDEC, 0xffffffffUL);
		/* disable vdec1 clock */
		...
		/* vdec1 power off */
		if (is_power_ctrl_ver2) {
			pwr_result = power_ctrl_sleep(0, 1);
			if (pwr_result)
				pr_err("vdec1 power off ctrl sleep fail.\n");
		} else {
			pwr_result = power_ctrl_sleep(0, 2);
			if (pwr_result)
				pr_err("vdec1 power off ctrl bit2 sleep fail.\n");
			pwr_result = power_ctrl_sleep(0, 3);
			if (pwr_result)
				pr_err("vdec1 power off ctrl bit3 sleep fail.\n");

		}
	} else if (core == VDEC_2) {
		if (has_vdec2()) {
			/* enable vdec2 isolation */
			pwr_result = power_ctrl_iso(0, 8);
			if (pwr_result)
				pr_err("vdec2 power off ctrl bit8 iso fail.\n");
			pwr_result = power_ctrl_iso(0, 9);
			if (pwr_result)
				pr_err("vdec2 power off ctrl bit9 iso fail.\n");
			/* power off vdec2 memories */
			WRITE_VREG(DOS_MEM_PD_VDEC2, 0xffffffffUL);
			/* disable vdec2 clock */
			vdec2_clock_off();
			...
			pwr_result = power_ctrl_sleep(0, 4);
			if (pwr_result)
				pr_err("vdec2 power off ctrl bit4 sleep fail.\n");
			pwr_result = power_ctrl_sleep(0, 5);
			if (pwr_result)
				pr_err("vdec2 power off ctrl bit5 sleep fail.\n");
		}
	} else if (core == VDEC_HCODEC) {
		if (has_hdec()) {
			/* enable hcodec isolation */
			if (is_power_ctrl_ver2) {
				pwr_result = power_ctrl_iso(0, 0);
				if (pwr_result)
					pr_err("hcodec power off ctrl iso fail.\n");
			} else {
				pwr_result = power_ctrl_iso(0, 4);
				if (pwr_result)
					pr_err("hcodec power off ctrl bit4 iso fail.\n");
				pwr_result = power_ctrl_iso(0, 5);
				if (pwr_result)
					pr_err("hcodec power off ctrl bit5 iso fail.\n");
			}
			/* power off hcodec memories */
			WRITE_VREG(DOS_MEM_PD_HCODEC, 0xffffffffUL);
			...
			/* hcodec power off */
			if (is_power_ctrl_ver2) {
				pwr_result = power_ctrl_sleep(0, 0);
				if (pwr_result)
					pr_err("hcodec power off ctrl sleep fail.\n");
			} else {
				pwr_result = power_ctrl_sleep(0, 0);
				if (pwr_result)
					pr_err("hcodec power off ctrl bit0 sleep fail.\n");
				pwr_result = power_ctrl_sleep(0, 1);
				if (pwr_result)
					pr_err("hcodec power off ctrl bit1 sleep fail.\n");
			}
		}
	} else if (core == VDEC_HEVC) {
		if (has_hevc_vdec()) {
			...
			if (no_powerdown == 0) {
				/* enable hevc isolation */
				if (is_power_ctrl_ver2) {
					pwr_result = power_ctrl_iso(0, 2);
					if (pwr_result)
						pr_err("hevc power off ctrl iso fail.\n");
				} else {
					pwr_result = power_ctrl_iso(0, 10);
					if (pwr_result)
						pr_err("hevc power off ctrl bit10 iso fail.\n");
					pwr_result = power_ctrl_iso(0, 11);
					if (pwr_result)
						pr_err("hevc power off ctrl bit11 iso fail.\n");
				}
			/* power off hevc memories */
			WRITE_VREG(DOS_MEM_PD_HEVC, 0xffffffffUL);
			...
			if (is_power_ctrl_ver2) {
				...
				pwr_result = power_ctrl_sleep(0, 6);
				if (pwr_result)
					pr_err("hevc power off ctrl bit6 sleep fail.\n");
				pwr_result = power_ctrl_sleep(0, 7);
				if (pwr_result)
					pr_err("hevc power off ctrl bit7 sleep fail.\n");
			}
