			...
			unsigned int start, unsigned int len)
{
	...
	break;
	case GEN_PWR_SLEEP0:
		if (power_ctrl_sleep(val ? 0 : 1, start))
			ge2d_log_err("set power GEN_PWR_SLEEP0 failed\n");
	break;
	case GEN_PWR_ISO0:
		if (power_ctrl_iso(val ? 0 : 1, start))
			ge2d_log_err("set power GEN_PWR_ISO0 failed\n");
	break;
	case MEM_PD_REG0:
		if (power_ctrl_mempd0(val ? 0 : 1, 0xFF, start))
			ge2d_log_err("set power GEN_PWR_ISO0 failed\n");
	break;
	default:
