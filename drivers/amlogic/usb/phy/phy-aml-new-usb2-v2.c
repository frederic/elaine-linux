...
void set_usb_phy_host_tuning(int port, int default_val)
{
	...
	pr_err("---%s port(%d) tuning for host cf(0x%x)--\n",
		default_val ? "Recovery" : "Set",
		port, readl(phy_reg_base + 0x38));
	if (!default_val) {
...
void set_usb_phy_device_tuning(int port, int default_val)
{
	...
	phy_reg_base = g_phy2_v2->phy_cfg[port];
	pr_err("---%s port(%d) tuning for device cf(0x%x)--\n",
		default_val ? "Recovery" : "Set",
		port, readl(phy_reg_base + 0x38));
	if (!default_val) {
...
{
	/* Powerup usb_comb */
	if (power_ctrl_sleep(1, phy->u2_ctrl_sleep_shift))
	...
	udelay(100);
	if (power_ctrl_iso(1, phy->u2_ctrl_iso_shift))
