...
static const char * const max98373_limiter_attack_rate_text[] = {
	...
	"40.96ms", "81.92ms", "163.84ms", "327.68ms"
};
...
SOC_SINGLE("Speaker Enable", MAX98373_R20FF_GLOBAL_SHDN, 0, 1, 0),
...
SOC_SINGLE_TLV("DHT Attack Step Volume", MAX98373_R20D2_DHT_ATTACK_CFG,
	MAX98373_DHT_ATTACK_STEP_SHIFT, 3, 0, max98373_dht_step_size_tlv),
SOC_SINGLE_TLV("DHT Release Step Volume", MAX98373_R20D3_DHT_RELEASE_CFG,
	MAX98373_DHT_RELEASE_STEP_SHIFT, 3, 0, max98373_dht_step_size_tlv),
...
static void max98373_reset(struct snd_soc_codec *codec)
{
	...
	while (true) {
		...
		ret = regmap_update_bits(max98373->regmap,
			...
			MAX98373_SOFT_RESET_MASK,
			MAX98373_SOFT_RESET);
		if (ret)
			dev_err(codec->dev,
				"Reset command failed. (ret:%d)\n", ret);
		...
		if (!ret) {
			dev_info(codec->dev,
				"Reset completed (retry:%d)\n", count);
			break;
		}

		if (++count > 25)	{
			dev_err(codec->dev, "Reset failed. (ret:%d)\n", ret);
			break;
		}
		msleep(20);
	}
