...
#define TAS5805M_CTRL_STATE_MASK        (0x03)
#define TAS5805M_CTRL_STATE_DEEP_SLEEP  (0x00)
#define TAS5805M_CTRL_STATE_HIGH_Z      (0x02)
#define TAS5805M_CTRL_STATE_PLAY        (0x03)

#define TAS5805M_MIN_AGAIN   (0)
#define TAS5805M_MAX_AGAIN   (31)

#define TAS5805M_AMP_ID_BITMASK (0x03)
#define TAS5805M_A1_ID          (0x01)
#define TAS5805M_B0_ID          (0x03)

#define RETRIES              (3)
#define CONFIG_CRC_CHECKSUM  (0x2D)

static const unsigned int tas5805m_ctrl_states[] = {
	TAS5805M_CTRL_STATE_DEEP_SLEEP,
	TAS5805M_CTRL_STATE_PLAY
};
...
static cfg_reg tas5805m_enable_agl_sequence[] = {
	{ 0x00, 0x00 },  // Page 0
	{ 0x7f, 0x8c },  // Book 8C
	{ 0x00, 0x2c },  // Page 2C
...
static cfg_reg tas5805m_disable_agl_sequence[] = {
	{ 0x00, 0x00 },  // Page 0
	{ 0x7f, 0x8c },  // Book 8C
	{ 0x00, 0x2c },  // Page 2C
...
static cfg_reg tas5805m_amp_id_in_sequence[] = {
	{ 0x00, 0x00 },  // Page 0
	{ 0x7f, 0x00 },  // Book 0
...
static cfg_reg tas5805m_amp_id_out_sequence[] = {
	{ 0x00, 0x00 },  // Page 0
	{ 0x7f, 0x00 },  // Book 0
...
static struct tas5825m_priv *tas5825m_priv_ptr;
...
static void tas5805m_set_volume(struct snd_soc_codec *codec, int vol)
{
	...
	tas5825m_switch_book(codec, TAS5805M_BOOK_8C, TAS5805M_PAGE_2A);
	//w 58 24 xx xx xx xx 
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (priv->init_done) {
		...
		tas5805m_set_volume(codec, priv->software_volume);
...
static int tas5805m_get_analog_gain(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *uinfo) {
	...
	mutex_lock(&priv->lock);
	tas5825m_switch_book(codec, TAS5805M_BOOK_00, TAS5805M_PAGE_00);
	again_reg_val = snd_soc_read(codec, TAS5805M_REG_54);
	uinfo->value.integer.value[0] = again_reg_val;
	mutex_unlock(&priv->lock);
	return 0;
}

static int tas5805m_put_analog_gain(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *uinfo) {
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tas5825m_priv *priv = snd_soc_codec_get_drvdata(codec);
	...
	int ret = 0;
	int user_val = uinfo->value.integer.value[0];

	if (user_val > TAS5805M_MAX_AGAIN || user_val < TAS5805M_MIN_AGAIN) {
		return -EINVAL;
	}
	...
	if (priv->init_done) {
		mutex_lock(&priv->lock);
		tas5825m_switch_book(codec, TAS5825M_BOOK_00, TAS5825M_PAGE_00);
		ret = snd_soc_update_bits(codec, TAS5805M_REG_54,
					  0x1F, user_val);
		if (ret < 0) {
			dev_err(dev, "Failed to Update Analog Gain 0x%02X\n",
				user_val);
			mutex_unlock(&priv->lock);
			return -EINVAL;
		}
...
static int tas5805m_get_play_mode(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *uinfo) {
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tas5825m_priv *priv = snd_soc_codec_get_drvdata(codec);
	unsigned int ctrl_state = 0;
	unsigned int is_play_mode = 0;

	mutex_lock(&priv->lock);
	tas5825m_switch_book(codec, TAS5805M_BOOK_00, TAS5805M_PAGE_00);
	ctrl_state = snd_soc_read(codec, TAS5805M_REG_03);
	ctrl_state &= TAS5805M_CTRL_STATE_MASK;
	is_play_mode = (ctrl_state == TAS5805M_CTRL_STATE_PLAY);
	uinfo->value.integer.value[0] = is_play_mode;
	mutex_unlock(&priv->lock);
	return 0;
}

static int tas5805m_put_play_mode(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *uinfo) {
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tas5825m_priv *priv = snd_soc_codec_get_drvdata(codec);
	struct i2c_client *client = priv->client;
	...
	if (uinfo->value.integer.value[0] > 1) {
		return -EINVAL;
	}

	ctrl_val = tas5805m_ctrl_states[uinfo->value.integer.value[0]];
	if (priv->init_done) {
		mutex_lock(&priv->lock);
		tas5825m_switch_book(codec, TAS5825M_BOOK_00, TAS5825M_PAGE_00);
		ret = snd_soc_update_bits(codec, TAS5805M_REG_03,
					   TAS5805M_CTRL_STATE_MASK,
					   TAS5805M_CTRL_STATE_HIGH_Z);
		if (ret < 0) {
			dev_err(dev, "Failed to Enter High Z Control State\n");
			mutex_unlock(&priv->lock);
			return -EINVAL;
		}
		ret = snd_soc_update_bits(codec, TAS5805M_REG_03,
					  TAS5805M_CTRL_STATE_MASK, ctrl_val);
		if (ret < 0) {
			dev_err(dev, "Failed to Enter Control State 0x%02X\n",
				ctrl_val);
			mutex_unlock(&priv->lock);
			return -EINVAL;
		}
...
static const struct snd_kcontrol_new tas5825m_snd_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("Speaker Enable", 0,
			    tas5805m_get_play_mode, tas5805m_put_play_mode),
	SOC_SINGLE_EXT("Analog Gain", TAS5805M_REG_54, 0, 0x1F, 0,
		       tas5805m_get_analog_gain, tas5805m_put_analog_gain),
};
...
void tas5805m_toggle_agl(int enable)
{
	struct i2c_client *client = tas5825m_priv_ptr->client;

	if (tas5825m_priv_ptr->init_done) {
		mutex_lock(&tas5825m_priv_ptr->lock);
		if (enable) {
			transmit_registers(client,
				tas5805m_enable_agl_sequence,
				ARRAY_SIZE(tas5805m_enable_agl_sequence));
		} else {
			transmit_registers(client,
				tas5805m_disable_agl_sequence,
				ARRAY_SIZE(tas5805m_disable_agl_sequence));
		}
		mutex_unlock(&tas5825m_priv_ptr->lock);
	} else {
		dev_info(&client->dev, "Init is not done\n");
	}
}
EXPORT_SYMBOL(tas5805m_toggle_agl);

static ssize_t tas5805m_amplifier_id_show(struct device *dev,
					  ...
					  char *buf)
{
	...
	if (!priv->init_done) {
		ret = snprintf(buf, PAGE_SIZE,
			       "ID Read Failed. Init Not Complete.\n");
		return ret;
	}

	mutex_lock(&priv->lock);
	transmit_registers(client,
			   tas5805m_amp_id_in_sequence,
			   ARRAY_SIZE(tas5805m_amp_id_in_sequence));
	i2c_read_reg(client, TAS5805M_REG_08, &reg_val);
	transmit_registers(client,
			   tas5805m_amp_id_out_sequence,
			   ARRAY_SIZE(tas5805m_amp_id_out_sequence));
	if ((reg_val & TAS5805M_AMP_ID_BITMASK) == TAS5805M_B0_ID)
		ret = snprintf(buf, PAGE_SIZE, "B0\n");
	else if ((reg_val & TAS5805M_AMP_ID_BITMASK) == TAS5805M_A1_ID)
		ret = snprintf(buf, PAGE_SIZE, "A1\n");
	else
		ret = snprintf(buf, PAGE_SIZE, "Unknown ID\n");
	mutex_unlock(&priv->lock);
	return ret;
}
static DEVICE_ATTR(amplifier_id, 0444, tas5805m_amplifier_id_show, NULL);

int tas5805m_create_sysfs(struct i2c_client *client)
{
	int ret = 0;

	ret = device_create_file(&client->dev, &dev_attr_amplifier_id);
	if (ret) {
		dev_err(&client->dev, "Failed to create device sysfs files");
		device_remove_file(&client->dev, &dev_attr_amplifier_id);
		return -ENOMEM;
	}
	dev_info(&client->dev, "Created device sysfs files");
	return ret;
}

static void tas5805m_priv_init(struct tas5825m_priv *priv)
{
	int i;
	u8 crc;

	if (priv->init_done) return;

	for (i = 0; i < RETRIES; i++) {
		/* reset CRC checksum */
		transmit_registers(priv->client, tas5805m_reset_checksum_sequence,
		...
		transmit_registers(priv->client, tas5805m_check_checksum_sequence,
				ARRAY_SIZE(tas5805m_check_checksum_sequence));
		i2c_read_reg(priv->client, TAS5805M_REG_7E, &crc);
		dev_info(&priv->client->dev, "CRC Checksum Value = 0x%02X,"
			 " Expected Checksum Value = 0x%02X",
			 crc, CONFIG_CRC_CHECKSUM);

		if (CONFIG_CRC_CHECKSUM == crc) {
			priv->init_done = 1;
			break;
		}
	}
...
static int tas5825m_prepare(struct snd_pcm_substream *substream,
...
static int tas5825m_probe(struct i2c_client *client, struct regmap *regmap)
{
	...
	tas5825m_priv_ptr = priv;

	priv->client = client;
	priv->regmap = regmap;
	priv->software_volume = 0;

	tas5805m_create_sysfs(client);
