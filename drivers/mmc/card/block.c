...
static struct fts_mmc_info gst_mmc_info;
static struct mmc_card *bl_card_test;
#endif
...
#ifdef CONFIG_AMLOGIC_MMC
void  get_mmc_card(struct fts_mmc_info **mmc_info, struct mmc_card **dest)
{
	*mmc_info = &gst_mmc_info;
	*dest = bl_card_test;
}
#endif
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	pr_err("%s() %d\n", __func__, __LINE__);
	gst_mmc_info.bit = card->csd.read_blkbits;
	gst_mmc_info.blk_size = 1 << card->csd.read_blkbits; /* size of a block */
	bl_card_test = card;
#endif
