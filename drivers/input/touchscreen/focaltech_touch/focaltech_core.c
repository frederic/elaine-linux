...
static const int NUM_BOE_PANEL_TYPES __initdata = 2;
static const int NUM_INX_PANEL_TYPES __initdata = 0;
static const char* focaltech_boe_panel_types[] __initdata =
                                                {"lcd_1", "lcd_3"};
static const char* focaltech_inx_panel_types[] __initdata = {};
#elif defined(CONFIG_TOUCHSCREEN_FTS_5726) && !defined(CONFIG_TOUCHSCREEN_FTS_3427)
static const int NUM_BOE_PANEL_TYPES __initdata = 2;
static const int NUM_INX_PANEL_TYPES __initdata = 2;
static const char* focaltech_boe_panel_types[] __initdata =
                                                {"boe_nvtk_10", "boe_fiti_10"};
static const char* focaltech_inx_panel_types[] __initdata =
                                                {"inx_nvtk_10", "inx_fiti_10"};
#endif

static enum fts_panel_vendor panel_vendor = FTS_PANEL_VENDOR_NONE;
static int maybe_focaltech __initdata = 0;
...
#if defined(CONFIG_TOUCHSCREEN_FTS_5726_P1)
            input_report_abs(data->input_dev, ABS_MT_POSITION_X, data->pdata->x_max - events[i].x);
            input_report_abs(data->input_dev, ABS_X, data->pdata->x_max - events[i].x);
#else
...
#if defined(CONFIG_TOUCHSCREEN_FTS_5726_P1)
    if (!strcmp(name, "focaltech,display-coords,boe,p1") ||
        !strcmp(name, "focaltech,display-coords,inx,p1")) {
...
#if defined(CONFIG_TOUCHSCREEN_FTS_5726_P1)
    if (panel_vendor == FTS_PANEL_VENDOR_BOE) {
        ret = fts_get_dt_coords(dev, "focaltech,display-coords,boe,p1", pdata);
    } else if (panel_vendor == FTS_PANEL_VENDOR_INX) {
        ret = fts_get_dt_coords(dev, "focaltech,display-coords,inx,p1", pdata);
    } else {
        ret = -EINVAL; // Unsupported panel type.
    }
#else
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ts_data->panel_vendor = panel_vendor;
    if (ts_data->panel_vendor == FTS_PANEL_VENDOR_NONE) {
        FTS_WARN("Unable to determine panel vendor");
    }
...
/*****************************************************************************
*  Name: get_panel_type
*  Brief: Use panel_type command line argument to determine if possibly
          using a FocalTech touch IC/sensor.
*  Input: panel_type command line argument
*  Output: nothing
*  Return: 0
*****************************************************************************/
...
{
    int i = 0;
    FTS_INFO("Panel type command line argument: %s", panel_arg);

    // Maybe a FocalTech IC if no arguments provided
    if (*panel_arg == 0) {
      FTS_WARN("No panel_type command line argument found");
      maybe_focaltech = 1;
      return 0;
    }

    // Check if panel_type argument matches expected BOE strings
    for (i = 0; i < NUM_BOE_PANEL_TYPES; i++) {
        if (strcmp(panel_arg, focaltech_boe_panel_types[i]) == 0) {
            FTS_INFO("Using a BOE display");
            panel_vendor = FTS_PANEL_VENDOR_BOE;
            maybe_focaltech = 1;
            return 0;
        }
    }

    // Check if panel_type argument matches expected INX strings
    // For the FTS_3427, INX panels do not use FocalTech ICs
    for (i = 0; i < NUM_INX_PANEL_TYPES; i++) {
        if (strcmp(panel_arg, focaltech_inx_panel_types[i]) == 0) {
            FTS_INFO("Using an INX display");
            panel_vendor = FTS_PANEL_VENDOR_INX;
#if defined(CONFIG_TOUCHSCREEN_FTS_3427)
            // Panels using the FT3427 IC only came from BOE
            maybe_focaltech = 0;
#else
            // Panels using the FT5726 IC came from both BOE and INX
            maybe_focaltech = 1;
#endif  // CONFIG_TOUCHSCREEN_FTS_3427
            return 0;
        }
    }
    return 0;
...
static int __init fts_ts_init(void)
{
    ...
    if (!maybe_focaltech)
    {
        FTS_INFO("Not using a FocalTech touch IC/sensor");
        return -ENODEV;
    }
