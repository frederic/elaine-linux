...
static const int NUM_PANEL_TYPES __initdata = 2;
static const char *goodix_panel_types[] __initdata = {"lcd_2", "lcd_4"};
static int maybe_goodix __initdata = 0;
...
static s32 gtp_request_enable_irq(struct goodix_ts_data *ts);
static void gtp_free_irq(struct goodix_ts_data *ts, int irq);
...
void gtp_irq_disable(struct goodix_ts_data *ts)
{
    ...
    if (!ts->irq_is_disable && ts->use_irq)
    {
        ts->irq_is_disable = 1;
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (touch_num > 1)
    {
        ...
        GTP_DEBUG("Multitouch detected with %d fingers", touch_num);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    // Attempt to free IRQ before using it as output (Error otherwise)
    gtp_free_irq(ts, client->irq);
    GTP_GPIO_OUTPUT(gtp_rst_gpio, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms

    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    ret = gpio_direction_output(gtp_int_gpio, client->addr == 0x14);
    if (ret)
    {
        gtp_request_enable_irq(ts);
        return;
    }
    ...
    // Request and enable IRQ after freeing it above for use as output
    gtp_request_enable_irq(ts);
}
 ...
 */
	...
	/* parse config data*/
#ifdef GTP_CONFIG_OF
	GTP_INFO("Get config data from device tree.");
...
#else
	GTP_INFO("Get config data from header file.");
...
#endif // GTP_COMPATIBLE_MODE
	{
	    ...
	    if (ret == SUCCESS) {
	        GTP_INFO("Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
...
#endif // GTP_DRIVER_SEND_CFG
        GTP_INFO("Pre-Swap X_MAX: %d, Pre-Swap Y_MAX: %d, TRIGGER: 0x%02x", ts->abs_x_max,ts->abs_y_max,ts->int_trigger_type);
    }
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    {
        GTP_ERROR("Unable to read IC version");
        return -ENODEV;
    }
    return ret;
}
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if ((ts->use_irq && ts->irq_is_free) || !ts->irq_was_requested)
    {
...
/*******************************************************
Function:
    Attempt to request and enable the IRQ
Input:
    ts: private data.
Output:
    Executive outcomes.
...
static s32 gtp_request_enable_irq(struct goodix_ts_data *ts)
{
    ...
    if (ts->irq_was_requested && ts->use_irq && ts->irq_is_free)
    {
        ret = gtp_request_irq(ts);
        if (ret)
        {
            GTP_ERROR("Request and Enable IRQ Failed. ErrNo: %d", ret);
            return ret;
        }
        gtp_irq_enable(ts);
    }
...
static void gtp_free_irq(struct goodix_ts_data *ts, int irq)
{
    GTP_INFO("Free the IRQ");
    if (ts->use_irq && !ts->irq_is_free) {
        free_irq(irq, ts);
        ts->irq_is_free = 1;
        ts->irq_is_disable = 1;
    }
...
static void gtp_parse_dt(struct device *dev)
{
  ...
  GTP_INFO("Parsing Goodix Device Tree");

	gtp_int_gpio = of_get_named_gpio(np, "goodix,irq-gpio", 0);
	gtp_rst_gpio = of_get_named_gpio(np, "goodix,rst-gpio", 0);
        GTP_INFO("gtp_int_gpio: %d", gtp_int_gpio);
        GTP_INFO("gtp_rst_gpio: %d", gtp_rst_gpio);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    // Init IRQ status-tracking variables
    ts->irq_was_requested = 0;
    ts->irq_is_disable = 1;
    ts->irq_is_free = 1;
    ts->use_irq = 0;
    ...
    GTP_INFO("Clock ticks for an esd cycle: %d", ts->clk_tick_cnt);
...
static s32 gtp_init_ext_watchdog(struct i2c_client *client)
{
    ...
    GTP_INFO("[Esd]Init external watchdog");
...
/*******************************************************
Function:
    Use commandline args to determine if possibly using a
    Goodix touch IC/sensor
Input:
    panel_type argument from command line
Output:
    0
********************************************************/
...
{
    int i = 0;
    GTP_INFO("Panel type command line argument: %s", panel_arg);

    // May be a Goodix IC if no argument provided
    if(*panel_arg == 0)
    {
      GTP_WARN("No panel_type command line argument found");
      maybe_goodix = 1;
      return 0;
    }

    // Check if arguments provided match expected for Goodix
    for(i = 0; i < NUM_PANEL_TYPES; i++)
    {
      if (strcmp(panel_arg, goodix_panel_types[i]) == 0)
      {
        maybe_goodix = 1;
      }
    }
...
static int __init goodix_ts_init(void)
{
    ...
    if (!maybe_goodix)
    {
        GTP_INFO("Not using a Goodix touch IC/sensor");
        return -ENODEV;
    }
