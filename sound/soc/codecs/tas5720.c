...
int tas5720_update_bits(struct snd_soc_codec *codec, unsigned short reg,
				 unsigned short bit, unsigned short value, const char* func)
{
	int ret;
	dev_dbg(codec->dev, "%s, update register: %x, bit: %x, to value: %x\n", func, reg, bit, value);
	ret = snd_soc_update_bits(codec, reg, bit, value);
	dev_dbg(codec->dev, "%s, reg %x value is %x\n", func, TAS5720_DIGITAL_CTRL1_REG, snd_soc_read(codec, TAS5720_DIGITAL_CTRL1_REG));
	return ret;
}
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (last_rate >= 88200 && rate < 88200) {
		dev_dbg(codec->dev, "reset when set to single speed from d-speed\n");

		/* Configure the TDM slot to process audio from */
		slot_val = snd_soc_read(codec, TAS5720_DIGITAL_CTRL2_REG);

		ret = tas5720_update_bits(codec, TAS5720_POWER_CTRL_REG,
				TAS5720_SDZ, 0, __func__);
		if (ret < 0)
			dev_err(codec->dev, "failed to wr PWR_CTRL reg:%d\n",
				ret);

		ret = tas5720_update_bits(codec, TAS5720_POWER_CTRL_REG,
				TAS5720_SDZ, TAS5720_SDZ, __func__);
		if (ret < 0)
			dev_err(codec->dev, "failed to wr PWR_CTRL reg:%d\n",
				ret);

		tas5720_update_bits(codec, TAS5720_DIGITAL_CTRL1_REG,
				  TAS5720_TDM_CFG_SRC, TAS5720_TDM_CFG_SRC, __func__);

		dev_dbg(codec->dev, "update register: %x, to value: %x\n",
				TAS5720_DIGITAL_CTRL2_REG, slot_val);
		snd_soc_write(codec, TAS5720_DIGITAL_CTRL2_REG, slot_val);
	}

	dev_dbg(codec->dev, "set speed mode bit to: %d\n", ssz_ds);
	ret = tas5720_update_bits(codec, TAS5720_DIGITAL_CTRL1_REG,
				  TAS5720_SSZ_DS, ssz_ds ? TAS5720_SSZ_DS : 0, __func__);
	if (ret < 0) {
				    ...
				    int slots, int slot_width)
{
	...
	/* Configure the TDM slot to process audio from */
	ret = tas5720_update_bits(codec, TAS5720_DIGITAL_CTRL2_REG,
				  TAS5720_TDM_SLOT_SEL_MASK, first_slot, __func__);
	if (ret < 0)
		goto error_snd_soc_update_bits;
...
static int tas5720_mute(struct snd_soc_dai *dai, int mute)
{
	...
	ret = tas5720_update_bits(codec, TAS5720_DIGITAL_CTRL2_REG,
				  TAS5720_MUTE, mute ? TAS5720_MUTE : 0, __func__);
	if (ret < 0) {
...
int tas5720_regmap_write_bits(struct regmap *regmap, struct snd_soc_codec *codec,
				 unsigned int reg, unsigned int bit, unsigned int val, const char* func)
{
	int ret;
	dev_dbg(codec->dev, "%s, update register: %x, bit: %x, to value: %x\n",
			func, reg, bit, val);
	ret = regmap_write_bits(regmap, reg, bit, val);
	dev_dbg(codec->dev, "%s, reg %x value is %x\n", func, TAS5720_DIGITAL_CTRL1_REG, snd_soc_read(codec, TAS5720_DIGITAL_CTRL1_REG));
	return ret;
}
...
static void tas5720_fault_check_work(struct work_struct *work)
{
	 ...
	 */
	ret = tas5720_regmap_write_bits(tas5720->regmap, tas5720->codec, TAS5720_POWER_CTRL_REG,
				TAS5720_SDZ, 0, __func__);
	if (ret < 0)
...
static int tas5720_codec_probe(struct snd_soc_codec *codec)
{
	...
	/* Set device to mute */
	ret = tas5720_update_bits(codec, TAS5720_DIGITAL_CTRL2_REG,
				  TAS5720_MUTE, TAS5720_MUTE, __func__);
	if (ret < 0)
		goto error_snd_soc_update_bits;
...
static bool tas5720_is_volatile_reg(struct device *dev, unsigned int reg)
{
	...
	case TAS5720_DIGITAL_CTRL1_REG:
		return true;
	default:
		return false;
	}
...
static const struct snd_kcontrol_new tas5720_snd_controls[] = {
	...
	SOC_SINGLE("Speaker Active", TAS5720_POWER_CTRL_REG, 0, 1, 0),
	SOC_SINGLE("TDM Auto Config Source", TAS5720_DIGITAL_CTRL1_REG, 6, 1, 1)
};
...
#define TAS5720_RATES	(SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_44100 |\
			 ...
			 const struct i2c_device_id *id)
{
	...
	/* Get the spk_sd gpio */
	//data->spk_sd_gpio = devm_gpiod_get(dev, "spk-sd", GPIOD_OUT_LOW);
	data->spk_sd_pin = of_get_named_gpio(dev->of_node, "spk-sd", 0);
	if (gpio_is_valid(data->spk_sd_pin)) {
		/* Set the direction as output */
		gpio_direction_output(data->spk_sd_pin, 1);
		gpio_set_value(data->spk_sd_pin, 1);
	}
	...
	/* Reset register file to default */
	ret = regmap_write_bits(data->regmap, TAS5720_POWER_CTRL_REG,
				TAS5720_SDZ, 0);
	if (ret < 0) {
		dev_err(dev, "failed to set SDZ:%d\n", ret);
	...
	ret = regmap_write_bits(data->regmap, TAS5720_POWER_CTRL_REG,
				TAS5720_SLEEP, 0);
	if (ret < 0) {
		dev_err(dev, "failed to reset SLEEP:%d\n", ret);
		snd_soc_unregister_codec(dev);
		return ret;
	}

	ret = regmap_write_bits(data->regmap, TAS5720_DIGITAL_CTRL1_REG,
				TAS5720_SSZ_DS, 0);
	if (ret < 0) {
		dev_err(dev, "failed to reset SSZ/DS:%d\n", ret);
		snd_soc_unregister_codec(dev);
		return ret;
	}
	...
	if (ret < 0) {
		dev_err(dev, "failed to reset SDZ:%d\n", ret);
		snd_soc_unregister_codec(dev);
		return ret;
	}
